...
def stock():
    try:
        ...
        # for i in range(265, 266):
            ...
            stock_data[list(stock_data.keys())[0]].append(i) 
            stock_data[list(stock_data.keys())[1]].append(stock)                                
            stock_data[list(stock_data.keys())[2]].append(code) 
...
def write_to_excel():
    ...
    #判斷是否有符號
    # for index, value in enumerate(stock_data[list(stock_data.keys())[4]]):
    #     punc = string.punctuation
    #     if value in punc:
    #         stock_data[list(stock_data.keys())[4]][index] = 888

    # for key, value in stock_data.items():
    #     print(key, len([item for item in value if item]))
            
    stock_df = pd.DataFrame(stock_data)
    # stock_df[[list(stock_data.keys())[4]]] = stock_df[[list(stock_data.keys())[4]]].astype('float32')
...
def payment(driver):
    ...
    if check_dividend(driver) == False:
        stock_data[list(stock_data.keys())[3]].append('')           
        stock_data[list(stock_data.keys())[6]].append('')     
    else:
        ...
        for dividend in dividend_total:
            ...
            if dividend.find_elements(By.TAG_NAME, 'div')[2].text.isdigit() == True:
                ...
                # print(dividend.find_elements(By.TAG_NAME, 'div')[3].text.strip(), dividend.find_elements(By.TAG_NAME, 'div')[5].text.strip())
                stock_data[list(stock_data.keys())[3]].append(dividend.find_elements(By.TAG_NAME, 'div')[2].text.strip())
                # stock_data[list(stock_data.keys())[4]].append(dividend.find_elements(By.TAG_NAME, 'div')[3].text.strip())
                # stock_data[list(stock_data.keys())[5]].append(dividend.find_elements(By.TAG_NAME, 'div')[4].text.strip())
                stock_data[list(stock_data.keys())[6]].append(dividend.find_elements(By.TAG_NAME, 'div')[5].text.strip())
                # stock_data[list(stock_data.keys())[7]].append(dividend.find_elements(By.TAG_NAME, 'div')[6].text.strip())
                # stock_data[list(stock_data.keys())[8]].append(dividend.find_elements(By.TAG_NAME, 'div')[7].text.strip())
                # stock_data[list(stock_data.keys())[9]].append(dividend.find_elements(By.TAG_NAME, 'div')[8].text.strip())
                # stock_data[list(stock_data.keys())[10]].append(dividend.find_elements(By.TAG_NAME, 'div')[9].text.strip())
                # stock_data[list(stock_data.keys())[11]].append(dividend.find_elements(By.TAG_NAME, 'div')[10].text.strip())
                # stock_data[list(stock_data.keys())[12]].append(dividend.find_elements(By.TAG_NAME, 'div')[11].text.strip())
                # stock_data[list(stock_data.keys())[13]].append(dividend.find_elements(By.TAG_NAME, 'div')[12].text.strip())
                # stock_data[list(stock_data.keys())[14]].append(dividend.find_elements(By.TAG_NAME, 'div')[13].text.strip())    
                implement_time += 1
            a += 1
            # 執行次數
            if implement_time == 5:
